...
// ---- 装配完整代理(mock 上游为 PROXY_UPSTREAM) ----
let mock, proxyUrl, httpServer;
...
test.before(async () => {
  ...
  httpServer = createHttpServer({ config, service, getToken });
  await new Promise(res => httpServer.server.listen(port, '127.0.0.1', res));
  proxyUrl = `http://127.0.0.1:${port}/v1/chat/completions`;
});

test.after(async () => {
  await new Promise(r => (httpServer ? httpServer.server.close(r) : r()));
  await new Promise(r => (mock ? mock.server.close(r) : r()));
  try { fs.rmSync(STATE_DIR, { recursive: true, force: true }); } catch (e) { /* 尽力清理 */ }
});

// 客户端视角的调用;body 返回 { status, json, text }
async function chat(stream, extra = {}) {
...
test('正常流式: 200 SSE 透传,内容与 [DONE] 完整', async () => {
...
test('正常非流式: 200 SSE 聚合为 completion', async () => {
  mock.set((req, res) => sse(res, [JSON.stringify(CH), JSON.stringify(USAGE)]));
...
test('非 2xx + SSE content-type + 标准 error 对象: 不得成为 200 假成功', async () => {
...
test('200 SSE 内嵌标准 error 对象帧: 翻译为 5xx 错误,不进聚合', async () => {
...
test('200 SSE 内嵌学校 errorMessage 帧: 429 繁忙语义(既有行为钉住)', async () => {
...
test('302 空响应体: 明确的会话失效文案,非"无法识别的响应"', async () => {
...
test('200 SSE 中途截断(无 [DONE]): 非流式 502 截断文案', async () => {
  mock.set((req, res) => {
